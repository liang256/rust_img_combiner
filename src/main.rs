...
fn main() -> Result<(), ImageDataErrors> {
    let args = Args::new();
    let (image_1, image_format_1) = find_imgae_from_path(args.image_1)?;
    let (image_2, image_format_2) = find_imgae_from_path(args.image_2)?;
...
fn find_imgae_from_path(path: String) -> Result<(DynamicImage, ImageFormat), ImageDataErrors> {
...
fn combine_images(image_1: DynamicImage, image_2: DynamicImage) -> Vec<u8> {
    let vec_1 = image_1.to_bgra8().to_vec();
    let vec_2 = image_2.to_bgra8().to_vec();
